...
module Autochef
  ...
  class LlmPlanner
    ...
    def attempt_llm_refinement(base_plan, pool:, freeform_note:, recent_plans:)
      ...
      if refined.nil?
        return Result.new(week_plan: base_plan, via_llm: false,
                          llm_error: 'LLM response failed validation — using deterministic plan.')
      end

      Result.new(week_plan: refined, via_llm: true, llm_error: nil)
    rescue StandardError => e
      Result.new(week_plan: base_plan, via_llm: false,
                 llm_error: "LLM call failed (#{e.class}: #{e.message}) — using deterministic plan.")
    end
